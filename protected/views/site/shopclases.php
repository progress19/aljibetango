  ...
  <section id="reservas-int" class="container shop-int"><br>
   ...
   <div class="col-md-12 fondo-shop">
	...
	<!-- ################################################################################################################ paso 1 -->	
			...
			<input type="hidden" name="todoslosdias" id="todoslosdias" value="<?php echo $clase->todoslosdias; ?>" />
			<input type="hidden" name="fecha_h" id="fecha_h" value="<?php echo $clase->fecha; ?>" />
			<input type="hidden" name="hasta" id="hasta" value="<?php echo $clase->hasta; ?>" />
			...
			<div class="tituloSeccion" id="tituloPaso1">
				<div class="col-md-8">	
					<h4><span class="dorada"><?php echo Yii::t('strings','Paso') ?> 1: </span><?php echo Yii::t('strings','Fecha de su visita')  ?>
						<val>
					<?php if ($clase->todoslosdias=='on') {}  

					  // una sola fecha	
					  if ($clase->fecha!='' AND $clase->hasta=='') {echo Yii::app()->dateFormatter->format("dd/MM/yyyy",$clase->fecha);}
				...
				<div class="col-md-4 right">
					<div class="cambiar" id="botonCambiarPaso1">
						<h5><a href="javascript:paso_clases(1,fecha)" class="dorada"><?php echo Yii::t('strings','CAMBIAR') ?></a></h5>
		        	</div>
	        	</div>
			</div>
			...
			<div id="conte-paso1">
				...
				<label for="asd" class="no-mov"><?php echo Yii::t('strings','Fecha') ?>: </label>
 					...
 					if ($clase->todoslosdias=='on') {$minDate = 1; $maxDate = '';}

 					if ($clase->fecha) {$minDate = 1; $maxDate = '';}   

 					if ($clase->fecha > 0 AND $clase->hasta > 0) {

						$minDate = Yii::app()->dateFormatter->format("dd/MM/yyyy", $clase->fecha);
 						$maxDate = Yii::app()->dateFormatter->format("dd/MM/yyyy", $clase->hasta);;
 					}
							  ...
							  $this->widget('zii.widgets.jui.CJuiDatePicker', array(
							       ...
							       'options'=>array(
						                ...
						                'onSelect' => 'js:function() {paso_clases(2)}',
						                'value' => '01-01-1978'
							       ),
							       'htmlOptions'=>array(
							        'class'=>'form-control'
				...
				<div class="col-xs-7 col-md-5 bot-sig-paso1">
					<a href="javascript:paso_clases(2)" class="boton_sig_shop pull-right"><?php echo Yii::t('strings','SIGUIENTE'); ?></a>
				</div>			
	...
	<!-- ################################################################################################################ paso 2 -->			
			...
			<div class="tituloSeccion" id="tituloPaso2">
				...
				<div class="cambiar" id="botonCambiarPaso2">
					<h5><a href="javascript:paso_clases(2)" class="dorada"><?php echo Yii::t('strings','CAMBIAR') ?></a></h5>
	        	</div>
	        	</div>
			</div>
				...
				<div id="conte-paso2">
					<div class="col-xs-5 col-md-7">
						...
						<select onchange="javascript:paso_clases(3,'<?php echo $titulo; ?>','<?php echo Yii::t('strings','Fecha') ?>','<?php echo Yii::t('strings','Personas') ?>')" name="cantidadPasajeros" id="cantidadPasajeros" class="form-control">
					...
					<div class="col-xs-7 col-md-5 bot-sig-paso2">
						<a href="javascript:paso_clases(3,'<?php echo $titulo; ?>','<?php echo Yii::t('strings','Fecha') ?>','<?php echo Yii::t('strings','Personas') ?>')" class="boton_sig_shop pull-right"><?php echo Yii::t('strings','SIGUIENTE'); ?></a>
					</div>
				</div> 
...
<input type="hidden" class="form-control" name="msg_enviadoOK" id="msg_enviandoOK" value="<?php echo Yii::t('strings', ' respuesta. Gracias!'); ?>">
	            ...
	            <input type="hidden" class="form-control" name="tipo" id="tipo" value="3"> <!-- 1:show / 2:degustacion 3:clase -->
	            ...
	            <input type="hidden" class="form-control" name="fechamax" id="fechamax" value="<?php if ($clase->todoslosdias=='on') {} else {
	            	echo Yii::app()->dateFormatter->format("dd/MM/yyyy",$clase->fecha);} ?>">
		...
		<div class="col-xs-12 col-md-6 no-padding fondo-cava">
			...
			<img src="<?php echo URLRAIZ.'/pics/clases/'.$clase->imagen ?>" alt="" class='img-responsive no-mov'>
			...
			<div class="conte-resumen">  <!-- contenedor col form + resumen -->
				...
				<div class="col-md-12" id="shopResumen">
					<div id="resumenFecha"><h4><?php echo Yii::t('strings','Fecha') ?> : <val>
						<?php if ($clase->todoslosdias=='on') {} else {
							echo Yii::app()->dateFormatter->format("dd/MM/yyyy",$clase->fecha);
							//echo Yii::app()->dateFormatter->formatDateTime($show->fecha, 'short', false);
							} ?>
					</val></h4></div>
					<div id="resumenPersonas"><h4><?php echo Yii::t('strings','Personas') ?> : <val></val></h4></div>
					<div id="resumenPersonaPrecio"><h4><?php echo Yii::t('strings','Precio por persona') ?> : USD <val><?php echo $clase->precio1 ?></val></h4></div>
